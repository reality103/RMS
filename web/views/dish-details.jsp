        ...
        <section class="py-5">
            <div class="container px-4 px-lg-5 my-5">
                <div class="row gx-4 gx-lg-5 align-items-center">
                        ...
                        <div class="carousel-inner" style="width: 100%; height: 100%;">
                            ...
                            <c:forEach items="${product.getImages()}" var="i" begin="1">
                                <div class="image-zoom carousel-item" style="width: 100%; height: 100%;">                              
                                    <img id="imgProduct" class=" d-block w-100" style="width: 100%; height: 100%;" src="${i.getUrl()}" alt="...">
                                </div>
                            </c:forEach>
                        </div>
                    ...
                    <div class="col-md-7">
                        <form action="carts" id="addcart" method="Post">
                            <h1>${requestScope.product.name}</h1>
                            <div class="group-status">
                                <span class="first_status">
                                    <span class="a_name">Tình trạng:</span> 
                                    <span class="status_name availabel"><i style="color:blue;">${requestScope.product.getIs_available() ? "Có sẵn":"Không còn"} </i></span> 
                                </span>
                            </div>
                            <div class="fs-5 mb-4">
                            ...
                            <div class="mb-5">${requestScope.product.description}</div>
                            ...
                            <div class="row">
                                ...
                                <c:if test="${sessionScope.user != null and sessionScope.user.getRole() ne 'admin'}" >
                                    <div class="col-lg-6">
                                        <input class="btn btn-outline-danger" type="submit" style="font-size: 20px" value="Đặt món">                                     
                                    </div>
                                </c:if>

                                <c:if test="${sessionScope.user == null}" >
                                    <div class="col-lg-6">
                                        <a class="btn btn-outline-danger" href="/SWP/login" style="font-size: 20px" >Đặt món</a>                                     
                                    </div>
                                </c:if>
                            </div>
                        ...
                        <c:forEach items="${productFeedbacks}" var="f">
                            <div class="col-sm-2">
                                <div style="width: 100px; height: 180px; border-radius: 50%; float: right;">
                                    <img  style="width: 100%; height: 100%;border-radius: 50%;  border: 1px solid #28a745;"src="${(f.avatar_user != null and f.avatar_user.length() > 0) ? f.avatar_user : "./images/user.png"}" alt="alt"/>
                                </div>
                            </div>
                            ...
                            <ul class="pagination">
                                <li class="page-item">
                                    <a <c:if test="${pageIndex!=1}">                         
                                            href="dish-details?id=${product.id}&cid=${product.category_id}&index=${pageIndex-1}"
                                ...
                                <c:forEach begin="1" end="${endPage}" var="i">
                                    <li class="page-item ${i==pageIndex ?"active" : ""}">
                                        <a class="page-link" href="dish-details?id=${product.id}&cid=${product.category_id}&index=${i}">${i}</a>
                                    </li>
                                </c:forEach>
                ...
                <div  class="related-products-slider row  justify-content-center ">
                    <c:forEach items="${relatedProducts}" var="p">                     
                        <div class="card col ms-2 mb-4 " style="background-color: white;  border-radius: 10px;  box-shadow: 0 0 10px rgba(0, 0, 0, 0.3);">                            
                            <!-- Product image-->
                            <img class="card-img-top mt-1"  style="height: 200px;"src="${p.images.get(0).getUrl()}" alt="..." />
                            <!-- Product details-->
                            <div class="card-body ">
                            ...
                            <div class="card-footer p-4 pt-0 border-top-0 bg-transparent ">
                                <div class="text-center"><a class="btn btn-outline-success mt-auto" href="dish-details?id=${p.id}&cid=${p.category_id}">Xem chi tiết</a></div>
                            </div>                        
                        </div>
                    </c:forEach>
                </div>
